...
def test_a_resolution_pass_writes_no_reading_episodes(store, monkeypatch):
    ...
    It was true, and it predated FRED. E1.3 says a resolution writes "the one
    ...
    With a key configured, FRED leads the resolution order, so every claim
    about a rate or an index would have pulled macro series into the corpus
    under `world:fred` — inflating the world share of the grounding mix §1
    rests on, with one narrow source.
    """
