...
- Threat-model acquisition, document parsing, prompt injection, model data
  ...
  `docs/threat-model.md` is that model, and it is what a Gate 6 security review
  reads against. Each entry names the control, the code holding it, the test
  proving it, and what the control does not cover;
  `tests/test_threat_model.py` checks that every cited module and test exists,
  so a control deleted from the code fails the document rather than quietly
