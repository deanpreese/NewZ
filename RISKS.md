...
## R-37 — Phase 3 red team: five confirmed faults in the phase it took one day to build. **2026-08-20.**

Nine epics, eight migrations and roughly sixty tests in one session, by one
agent, with no review between epics beyond a green gate. The gate is a poor
...
every keyword and is admitted:

> *"This page is NOT written by a digital being. It is generated by a person,
> and no human hand edits them is false."*

That renders. Substring presence is not a claim, and §9's commitment to no
undisclosed impersonation is the one place in this system where a guard that
...
`changed_paths()` reads `git diff` plus `git ls-files --others
--exclude-standard`, and an ignored file appears in neither. `hard_core.contains(".env")`
is `False`.

So the loop could flip reach by writing `.env`, and nothing in the freeze,
the drift check or the gate would see the diff — because there is no diff.
...
E3.7 computes each derivation's grade from `DERIVED_FROM`, and nothing checks
that the declaration matches the implementation:

| metric | declares | actually reads |
|---|---|---|
| `consequence_rate` | `advance_acceptance`, `claims_opened` | `concern_advances`, `resolutions` |
| `autonomy_against_world_grounding` | `advances_offered`, `self_grounding_share` | `concern_advances`, `works`, `self_grounding_share` |

The grades happen to come out defensible, and that is luck rather than design.
A declaration nobody checks is the E2.11 problem — a copy of a thing, drifting
from it — inside the module built to stop grades being lost in composition.

### R-37d — Agreement pairs one reply with every message it answered. **Medium.**

`_pairs` takes the next outbound message after each inbound one. The ambient
loop coalesces pending messages into a single reply and logs when it does. Three
messages answered by one reply become three exchanges judged against the same
text, inflating whatever the denominator says.

Also worth stating plainly: the classifier runs on AMBIENT, the same model
family as the being. The being is judged by itself. `model-graded` records that
a model judged it and not that it was **that** model.

### R-37e — Nothing regenerates the surface, and an empty backup verifies. **Low, and the first is embarrassing.**

Phase 3 built five tools and zero rhythms. The generator, `--verify`,
`rebuild_check` and `freeze_check` all run only when a person types them, so the
published surface is stale from the moment the being writes anything — and it
wrote a piece at 08:45 today. `SurfaceScheduler` appears in `run_newz.py` and is
the *noticing* surface, which made this harder to spot than it should have been.

And `cleanroom.verify()` counts rows without asserting any: a backup with zero
episodes returns 0 and passes. A backup that restores to nothing is exactly the
backup worth catching.

### What this says about the day

Every one of these is the same class of error: **a check that reads like a
guarantee and tests something narrower than it claims.** Keyword presence for a
claim. A git diff for a setting outside git. A declaration for an
implementation. A row count for a restorable backup.

The tests all pass, and they pass for reasons narrower than their names. That is
what a green gate cannot tell you, and it is the argument for E8.0 being the
next thing built rather than the fourth.
